...
export const TypeCard = ({ types, isSeeThrough = false, children }) => {
    ...
    const cardToColour3 = types.length === 1 ? colourPicker("to", types[0].type.name) : colourPicker("to", types[1].type.name);
    ...
    return (
        <div className="relative w-full my-1">
            ...
            <div className={`absolute inset-0 w-full rounded-lg bg-gradient-to-b ${cardToColour2} ` + cardFromColour2}></div>
            <div className={"relative" + gradientColours + cardToColour3}>
                {children}
            </div>
        </div>
    );
